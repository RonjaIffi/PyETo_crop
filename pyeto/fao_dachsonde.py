...
changed by Ronja Iffland


crop specific:    
def ra(d, zom, zoh, ws, h=xxx, zh=xxx, zm=xxx, k=0.41): zh(height), zm(height)
def rs(h=xxx): h(crop)
def d(h=xxx): h(crop)
def zom(h=xxx): h(crop)
def net_in_sol_rad(sol_rad, albedo=xxx): albedo(crop)
def LAIactive(LAI): LAI(crop)


sedum: rs und h aus Cirkel et al 2018
"""
...
# ab hier speziell für Ronjas Gründach Simulation
...
#    ro = pa * 1000 / (287 * t)
#    a1 = delta_svp*1000 * (net_rad - shf)*1000000/86400 + ro * cp*1000 * ((svp-avp)*1000)/ra
#    a2 = delta_svp*1000 + psy*1000 * (1 + (rs/ra))
#    a3 = 1 / (lat*1000)
#    
#    return a1 / a2 * a3 * 86400
...
# bis hier speziell für Ronjas Gründach Simulation
...
def wind_speed_3m(ws, z=10):
    '''
    :param ws: Measured wind speed [m s-1]
    :param z: Height of wind measurement above ground surface [m]
    :return: Wind speed at 3 m above the surface [m s-1]
    :rtype: float
    '''
